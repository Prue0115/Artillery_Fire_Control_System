...
* `get_version()`은 `afcs/VERSION`이 존재하면 내용을 반환하고, 없을 경우 기본 버전(`1.25.4`)을 기록합니다.
...
* `afcs/VERSION` 파일을 유지하면 소스 코드를 건드리지 않고도 배포물/설치 환경에서 버전을 갱신하거나 일관되게 노출할 수 있으므로 기본 상수만 두는 것보다 안전합니다.
