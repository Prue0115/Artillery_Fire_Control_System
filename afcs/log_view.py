"""Utility helpers for rendering and updating calculation logs."""
...
def render_log(log_body: ttk.Frame, entries, equipment_filter: str):
    ...
    for idx, entry in enumerate(filtered_entries):
        ...
        for idx_col, (label_text, width) in enumerate(
            (("CH", ui_theme.CH_WIDTH), ("MILL", ui_theme.MILL_WIDTH), ("ETA", ui_theme.ETA_WIDTH))
        ):
        ...
        for row_idx, charge in enumerate(charges, start=2):
            ...
            for col_offset, key, width in (
                (0, "charge", ui_theme.CH_WIDTH),
                (1, "mill", ui_theme.MILL_WIDTH),
                (2, "eta", ui_theme.ETA_WIDTH),
            ):
