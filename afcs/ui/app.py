...
class AFCSApplication:
    ...
    def _build_layout(self) -> None:
        ...
        self.header = self._build_header(main)
        self.inputs = self._build_inputs(main)
        self.tables, self.delta_label = self._build_results(main)
        self.theme_toggle, self.log_toggle_button = self._build_bottom_bar(main)
    ...
    def _build_bottom_bar(self, parent: ttk.Frame) -> tuple[ttk.Button, ttk.Button]:
        ...
        try:
            light_icon = tk.PhotoImage(file=str(ui_theme.ICONS_DIR / "Light Mode.png"))
            dark_icon = tk.PhotoImage(file=str(ui_theme.ICONS_DIR / "Dark Mode.png"))
        except Exception:
            light_icon = dark_icon = None
        ...
        theme_toggle = ttk.Button(
            bottom_bar,
            text="" if light_icon else "🌞",
            style="ThemeToggle.TButton",
            image=light_icon if light_icon else None,
            cursor="hand2",
