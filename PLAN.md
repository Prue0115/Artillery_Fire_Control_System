...
Python 의존성 없이 C만으로 계산기를 제공한다. CLI 단일 바이너리를 중심으로 구축하고, macOS 우선의 C GUI 인스톨러(선택적 바로가기 생성 포함)를 함께 배포한다.
...
- **GUI 인스톨러**: `installer_gui` 실행 시 macOS의 `osascript` 네이티브 다이얼로그를 우선 사용해 설치 경로 지정, 바로가기 생성 여부를 묻고, 선택 내용을 적용해 바이너리·테이블·런처를 배포한다. 다른 OS에서는 동일한 흐름을 CLI 프롬프트로 폴백한다.
