...
def find_solutions(
    ...
    for charge in charges:
        ...
        if filter_fn:
            solution, was_filtered = filter_fn(solution)
            filtered_out = filtered_out or was_filtered
            if solution is None:
                continue
...
def enforce_m1129_elevation_limits(solution):
    min_mill = M1129_MIN_DEG * MIL_PER_DEG
    max_mill = M1129_MAX_DEG * MIL_PER_DEG
    mill = solution["mill"]
    if mill < min_mill or mill > max_mill:
        return None, True
